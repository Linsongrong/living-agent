...
除了长时间静默，Agent 现在有了**即时回血**途径：

**微触发发呆**：
- 条件：能量在 30-60% 之间
- 概率：30%
- 效果：恢复 +10 能量，情绪变为 chill
...
**多巴胺奖励**：
- 条件：探索获得高质量发现
- 效果：恢复 +25 能量，情绪重置为 curious

这样 Agent 白天不再单向掉血，而是有多种恢复途径！
...
# 碎片化回血命令
python -m src.vital_signs.py daydream    # 发呆检测 (30%概率+10)
python -m src.vital_signs.py dopamine    # 多巴胺奖励 (+25)
python -m src.vital_signs.py chat-recover  # 闲聊恢复 (+5)

# 获取前台 prompt 片段（自动检测闲聊）
python -m src.inject_soul.py inject
```
